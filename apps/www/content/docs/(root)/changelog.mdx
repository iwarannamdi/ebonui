...
We've upgraded [ui.shadcn.com](https://ui.shadcn.com) to Next.js 15.3 and Tailwind v4. The site now uses the upgraded `ebonui` components.
...
- We're deprecating the default style. New projects will use ebonui.
...
An update on icons. The ebonui style now uses Lucide as the default icon set.
...
We are shipping two styles: `default` and `ebonui` (with more coming soon).
...
The `ebonui` style is a new style. It ships with smaller buttons, cards with shadows and a new set of icons from [Radix Icons](https://icons.radix-ui.com).
